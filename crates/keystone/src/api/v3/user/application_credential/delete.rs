...
#[utoipa::path(
    ...
    responses(
        (status = NO_CONTENT, description = "Application credential deleted"),
...
mod tests {
    ...
    -> openstack_keystone_core_types::application_credential::ApplicationCredential {
        openstack_keystone_core_types::application_credential::ApplicationCredentialBuilder::default()
            .id("cred-id").name("test-cred").user_id("uid")
            .project_id("pid").unrestricted(false).roles(vec![])
            .build().unwrap()
    }
    ...
    async fn test_delete() {
        ...
        let response = openapi_router()
            ...
            .oneshot(
                Request::builder()
                    .method("DELETE")
                    .uri("/users/uid/application_credentials/cred-id")
                    .extension(vsc)
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    async fn test_delete_credential_not_found() {
        ...
        let response = openapi_router()
            ...
            .oneshot(
                Request::builder()
                    .method("DELETE")
                    .uri("/users/uid/application_credentials/non-existing")
                    .extension(vsc)
                    .body(Body::empty())
                    .unwrap(),
            )
